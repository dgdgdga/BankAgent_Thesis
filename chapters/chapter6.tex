...
\subsection{User Center and RBAC}

The user center (user service, port 8081) integrates identity management, access control, department-level data isolation, as well as cross-cutting notification and document services. Table \ref{tab:pseudocode-login} shows the dual-mode login and JWT session management flow.
...
\small\singlespacing
\centering
\caption{Pseudocode --- Dual-Mode Login and JWT Session Management}
\label{tab:pseudocode-login}
\vspace{2mm}
...
\subsection{Task Center and Orchestration}
...
\subsubsection{Task Lifecycle and State Machine}

Every piece of work in the platform is modeled as a task record with a four-state lifecycle, shown in Table \ref{tab:task-lifecycle}.

\begin{table}[H]
\small\singlespacing
\centering
\caption{Task Lifecycle State Machine}
\label{tab:task-lifecycle}
...
\subsubsection{Submission, Queuing, and Thread Pool}
...
Table \ref{tab:task-submit} shows the asynchronous task submission and queuing mechanism.

\vspace{3mm}
\begin{table}[H]
\small\singlespacing
\centering
\caption{Pseudocode --- submitTask() Asynchronous Task Submission and Queuing}
\label{tab:task-submit}
\vspace{2mm}

\begin{lstlisting}
FUNCTION submitTask(username, rolesHeader, taskType, input)
    userId = getUserIdByUsername(username)

    // Step 1: Create and persist database record
    ...
    // Step 2: Submit to thread pool for async execution
    wsTaskId = String.valueOf(record.id)
    TRY:
        executor.execute(() ->
            executeWithRetry(record, wsTaskId,
                             username, rolesHeader))
        RETURN record
    CATCH RejectedExecutionException:
        // Queue full: mark failed, notify user
        record.status = "FAIL"
        record.errorMsg = "System busy."
        record.updatedAt = now()
        taskRecordMapper.updateById(record)
        sendProgress(wsTaskId, 100, "error",
...
\subsubsection{Agent Routing and Execution}

Once a worker thread picks up a task, the \texttt{executeTaskOnce()} method routes it to the appropriate agent based on \texttt{taskType}, shown in Table \ref{tab:task-routing}.

\begin{table}[H]
\small\singlespacing
\centering
\caption{Task Type to Agent Routing}
\label{tab:task-routing}
...
All downstream calls use Spring's \texttt{RestTemplate} with a 5-second connect timeout and 30-second read timeout. User identity headers (\texttt{X-User-Id}, \texttt{X-User-Name}, \texttt{X-User-Dept-Id}, \texttt{X-User-Roles}, and \texttt{X-User-Security-Level}) are forwarded so that each agent can apply its own authorization logic. This is especially important for RAG tasks because the query service uses these headers to build the permission snapshot before vector candidates are allowed into the LLM prompt. Service URIs are resolved from environment variables --- \texttt{CODE\_SERVICE\_URI}, \texttt{TOOL\_SERVICE\_URI}, \texttt{RAG\_SERVICE\_URI} --- with localhost fallbacks for development.

\subsubsection{Retry with Exponential Backoff}

Network calls to downstream agents can fail for transient reasons: a container restarting, a brief network hiccup, an LLM API rate limit. The retry mechanism in Table \ref{tab:retry-backoff} handles these gracefully.

\vspace{3mm}
\begin{table}[H]
\small\singlespacing
\centering
\caption{Pseudocode --- executeWithRetry() Exponential Backoff Loop}
\label{tab:retry-backoff}
\vspace{2mm}

\begin{lstlisting}
FUNCTION executeWithRetry(record, wsTaskId, username,
                          rolesHeader)
    ...
    WHILE attempt < MAX_RETRY:
        ...
        TRY:
            executeTaskOnce(record, wsTaskId,
                            username, rolesHeader)
            RETURN  // success
        CATCH Exception e:
            lastException = e
            IF attempt < MAX_RETRY:
                sendProgress(wsTaskId, 5, "running",
                    "Retrying... (" + attempt
                    + "/" + MAX_RETRY + ")")
                sleep(1000 * attempt)
            END IF
    ...
    record.status = "FAIL"
    record.errorMsg = lastException.message
    record.elapsedTime = elapsedSince(record.createdAt)
    record.updatedAt = now()
    taskRecordMapper.updateById(record)
    sendProgress(wsTaskId, 100, "error",
        "Execution failed after " + MAX_RETRY
        + " attempts")
END FUNCTION
\end{lstlisting}
\end{table}
...
\subsubsection{Progress Streaming and WebSocket Integration}
...
RAG tasks use the same orchestration path. When the submitted task type is RAG, the task service forwards the question to rag-agent through \texttt{POST /rag/query}, stores the returned answer, citations, retrieved document IDs, blocked document IDs, trace ID, and latency in \texttt{task\_record.output}, and pushes progress messages back to the frontend. This allows the Copilot, Task Center, and the dedicated RAG workbench to share the same backend capability instead of maintaining separate query implementations.
...
Table \ref{tab:pseudocode-websocket} shows the WebSocket-based task progress streaming mechanism with step-by-step JSON push and frontend rendering.
...
\small\singlespacing
\centering
\caption{Pseudocode --- TaskProgressWebSocketHandler Real-Time Progress Streaming}
\label{tab:pseudocode-websocket}
\vspace{2mm}
...
Figure \ref{fig:docker-topology} shows the Docker Compose service topology, illustrating how each service (Gateway, four backend microservices, frontend, MySQL, Redis, and optionally Milvus with etcd and MinIO) runs in an independent container on an internal Docker network, with only the Gateway port exposed externally.
