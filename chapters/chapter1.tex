...
This work presents BankAgent, a multi-agent AI platform built on on-premise infrastructure and designed for banking operations. Table 1 summarizes the project objectives across all layers of the platform.

\vspace{3mm}
\noindent\textbf{Table 1: Project Objectives by Platform Layer}
\vspace{2mm}
...
\midrule
\multirow{3}{*}{AI Agents} & Code Agent & Convert natural language to SQL through a Python LLM inference server, with five-layer whitelist validation ensuring only safe queries reach the database. \\
\cmidrule{2-3}
 & RAG Agent & Provide citation-backed answers to policy and procedural questions using Milvus vector retrieval and LLM generation, with security-level and department-based permission filtering. \\
\cmidrule{2-3}
 & Tool Agent & Automate business tasks --- meeting room booking, schedule conflict detection via Redis sorted sets, and route planning via Amap API --- through a unified natural language interface with DeepSeek intent routing. \\
\midrule
Intelligent Dispatch & Copilot & Sit above all three agents as a cross-page dispatcher, classifying user intent through a three-tier pipeline and routing requests to the appropriate agent. \\
\midrule
\multirow{5}{*}{Platform Services} & Auth \& RBAC & Provide JWT-based authentication with three-tier role-based access control (Admin, Department Admin, End-User) and department-level data isolation. \\
\cmidrule{2-3}
 & API Gateway & Serve as the single entry point with JWT global filtering, Redis-based rate limiting, and single-session enforcement. \\
\cmidrule{2-3}
 & Notification System & Handle multi-type notifications (chat, RAG access requests, SQL audit alerts) with a five-state state machine and human-in-the-loop approval workflow. \\
\cmidrule{2-3}
 & Document Management & Provide MinIO-based document storage with three security clearance levels, HITL escalation for cross-clearance access, and RAG index synchronization. \\
\cmidrule{2-3}
 & Task Center & Orchestrate asynchronous agent execution with a four-state lifecycle, exponential-backoff retry, and WebSocket-based real-time progress streaming. \\
\midrule
DevOps & Containerized Deploy & Deploy all 13 services via Docker Compose with a single command, including Nginx reverse proxy, Let's Encrypt HTTPS, and health-check-based startup ordering. \\
\bottomrule
\end{tabularx}
\end{table}
