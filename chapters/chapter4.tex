...
\begin{tikzpicture}[
  ...
  layerlabel/.style={rectangle, draw=none, font=\normalsize\bfseries, align=center, text width=2.5cm, inner sep=0pt},
  service/.style={rectangle, draw=black!70, fill=black!8, rounded corners=3pt, minimum width=2.2cm, minimum height=0.85cm, font=\small\bfseries, align=center, inner sep=6pt},
  sub/.style={rectangle, draw=black!50, fill=black!3, rounded corners=2pt, minimum width=2.2cm, minimum height=0.65cm, font=\footnotesize, align=center, inner sep=4pt},
  port/.style={font=\scriptsize\ttfamily},
  arrow/.style={->, >=stealth, line width=0.6pt},
  fainta/.style={->, >=stealth, line width=0.3pt, draw=black!40},
]
...
% === Layer 1: Web ===
\node[layerlabel] (L1) at (-0.5,0) {Layer 1\\\textbf{Web}};
\node[service, right=1.2cm of L1] (web) {Vue 3 SPA\\\scriptsize (Nginx :443)};
\node[font=\footnotesize, above=0.2cm of web] (user) {Browser};
\draw[arrow] (user) -- (web);

% === Layer 2: Gateway ===
\node[layerlabel] (L2) at (-0.5,-2.0) {Layer 2\\\textbf{Gateway}};
\node[service, right=1.2cm of L2, minimum width=5.0cm] (gw) {Spring Cloud Gateway\\\scriptsize (JWT Auth \textbullet{} Rate Limit \textbullet{} :8080)};

\draw[arrow] (web.south) -- (gw.north);

% === Layer 3: Business ===
\node[layerlabel, text height=1.5cm] (L3) at (-0.5,-4.3) {Layer 3\\\textbf{Business}};
\node[service, right=1.2cm of L3] (userSvc) {User\\Service\\\scriptsize :8081};
\node[service, right=0.5cm of userSvc] (taskSvc) {Task\\Service\\\scriptsize :8082};

\draw[arrow] (gw.south) -- ++(0,-0.3) -| (userSvc.north);
\draw[arrow] (gw.south) -- ++(0,-0.3) -| (taskSvc.north);

% === Layer 4: AI ===
\node[layerlabel, text height=2.0cm] (L4) at (-0.5,-7.0) {Layer 4\\\textbf{AI}};

\node[service, right=1.2cm of L4] (toolA) {Tool Agent\\\scriptsize :8083};
\node[service, right=0.5cm of toolA] (codeA) {Code Agent\\\scriptsize :8084};
\node[service, right=0.5cm of codeA] (ragA) {RAG Agent\\\scriptsize :8085};

% Python sub-services
\node[sub, below=0.4cm of codeA] (pyCode) {Python LLM Inference\\\scriptsize :8090};
\node[sub, below=0.4cm of ragA] (pyRag) {Python Embed Worker\\\scriptsize :8091};

\draw[arrow] (userSvc.south) -- ++(0,-0.2) -| (toolA.north);
\draw[arrow] (taskSvc.south) -- ++(0,-0.2) -| (codeA.north);
\draw[arrow] (taskSvc.south) -- ++(0,-0.2) -| (ragA.north);

\draw[arrow] (codeA.south) -- (pyCode.north);
\draw[arrow] (ragA.south) -- (pyRag.north);

% === Layer 5: Data ===
\node[layerlabel, text height=1.0cm] (L5) at (-0.5,-10.2) {Layer 5\\\textbf{Data}};

\node[sub, right=1.2cm of L5] (mysql) {MySQL\\\scriptsize :3306};
\node[sub, right=0.35cm of mysql] (redis) {Redis\\\scriptsize :6379};
\node[sub, right=0.35cm of redis] (milvus) {Milvus\\\scriptsize :9091};
\node[sub, right=0.35cm of milvus] (minio) {MinIO\\\scriptsize :9000};
\node[sub, right=0.35cm of minio] (etcd) {etcd\\\scriptsize :2379};

% Data connections
\draw[fainta] (pyCode.south) |- (mysql.north);
\draw[fainta] (pyRag.south) |- (milvus.north);
\draw[fainta] (userSvc.south east) -- ++(0.3,-1.5) -| (mysql.north);
\draw[fainta] (toolA.south) |- (redis.north);
\draw[fainta] (ragA.south east) -- ++(0.3,-0.8) -| (minio.north);

% Left thick line
\draw[line width=1pt, draw=black!50] (-3.0,0.3) -- (-3.0,-10.7);
