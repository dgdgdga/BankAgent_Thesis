...
\textbf{(6) Map and Loading Feedback.} Map rendering uses the Amap JS API for route display. Loading states are handled by a full-screen overlay with a rotating status message. The progress bar stays below 100 percent until the task actually completes, so the user never feels the interface has stalled.
