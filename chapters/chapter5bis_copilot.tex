...
% ======================================================================
\subhead{5.4 Copilot --- A Unified Intelligent Dispatcher Above the Three Agents}
...
\subhead{5.4.1 Design Rationale}
...
Table 28 summarizes the Copilot's position relative to the three agents.

\vspace{3mm}
\noindent\textbf{Table 28: Copilot vs. Three Domain Agents --- Role Comparison}
\vspace{2mm}
...
\subhead{5.4.2 Architecture Overview}
...
Figure 11 shows how a user query flows through the Copilot system.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig05_tool_agent_sequence.png}
\caption{Figure 11: Copilot Query Flow --- From Natural Language to Agent Execution}
\end{figure}
...
\subhead{5.4.3 Three-Tier Intent Classification}

The Python Flask \texttt{/route} endpoint is the Copilot's brain. It implements a three-tier classification strategy that balances cost, latency, and accuracy, shown in Table 29.

\vspace{3mm}
\noindent\textbf{Table 29: Three-Tier Intent Classification Strategy}
\vspace{2mm}

\begin{table}[H]
\centering
\begin{tabular}{@{}p{1.5cm}p{3.2cm}p{3.5cm}p{3.8cm}@{}}
\toprule
...
Tier 2 & Keyword greeting match & ``Hello'', ``What can you do'', ``你是谁'' & Zero \\
...
Tier 3b activates only when the LLM API is unreachable. A pure keyword-matching fallback scans for Chinese and English terms associated with each intent (e.g., ``SQL'', ``查询'', ``统计'' for CODE; ``会议室'', ``预订'', ``日程'' for TOOL) and defaults to RAG when no keywords match. This ensures the Copilot degrades gracefully rather than failing completely.

\subhead{5.4.4 Frontend Dispatch and User Experience}

After the intent is classified, the frontend widget takes over with type-specific handling, summarized in Table 30.

\vspace{3mm}
\noindent\textbf{Table 30: Intent Dispatch --- Frontend Behavior by Classification Result}
\vspace{2mm}

\begin{table}[H]
\centering
\begin{tabular}{@{}p{2.2cm}p{4.5cm}p{5.3cm}@{}}
\toprule
...
\subhead{5.4.5 Multi-Turn Clarification}
...
\subhead{5.4.6 Cross-Page Communication}
...
\begin{enumerate}
  \item \textbf{Browser Custom Events}: The widget dispatches \texttt{CustomEvent} objects on the \texttt{window} object --- \texttt{copilot-tool-result} for Tool Agent results, \texttt{copilot-code-query} for Code Agent queries. Target page components listen for these events and react accordingly.
  \item \textbf{localStorage}: As a safety net for pages that mount \textit{after} the event was dispatched (e.g., the widget navigated to a new page before the subscribing component existed), results are also written to localStorage keys. The target page checks these keys on mount and processes any pending result.
\end{enumerate}

\subhead{5.4.7 Second-Level Intent Classification in the Tool Agent}
...
\subhead{5.4.8 AI Provider Configuration Unification}
