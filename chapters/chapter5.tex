...
4 -- Vector Retrieval & VectorStoreService / \newline REST Milvus adapter & ANN search with COSINE similarity and a 4x over-fetch candidate size & Returns candidate chunks that are not trusted yet \\
...
6 -- Grounded Generation & Query prompt logic + \newline RagLlmClient & Build evidence-only prompt; call OpenAI-compatible chat-completions endpoint & Produces answer text and citation references from allowed chunks only \\
...
\addlinespace[1mm]
Query &
JWT headers $\rightarrow$ RagPermissionService $\rightarrow$ query embedding $\rightarrow$ Milvus over-fetch $\rightarrow$ permission filter $\rightarrow$ prompt builder $\rightarrow$ LLM &
...
Figure \ref{fig:rag-permission-gate} visualizes the security gate inside the query flow. The important point is that vector similarity only proposes candidates. The permission snapshot decides which chunks can become evidence. Blocked chunks are counted and logged, but their text is never copied into the LLM prompt or frontend citation panel.
...
\textbf{Vector candidates} & \textbf{Permission gate} & \textbf{Allowed output} \\
\midrule
Milvus returns over-fetched chunks with document IDs, chunk IDs, similarity scores, and security labels. &
Permission service compares each candidate with department scope, clearance level, admin role, and approved access requests. &
...
Unauthorized candidates are removed after retrieval and before prompt construction. &
...
\caption{RAG Permission Gate --- Similarity Candidates Become Evidence Only After Authorization}
...
The answer generation step uses an OpenAI-compatible chat-completions endpoint configured by \texttt{RAG\_LLM\_API\_KEY}, \texttt{RAG\_LLM\_BASE\_URL}, and \texttt{RAG\_LLM\_MODEL}. The prompt has three parts: the user question, a numbered context block containing permission-safe chunks, and answer rules. The answer rules require the model to answer only from the provided context, cite chunk identifiers, state that no evidence was found when the context is insufficient, and never infer content from blocked documents. The response returned to the frontend contains traceId, answer text, citations, retrieved chunks, retrieved document IDs, blocked document IDs, topK, status, and latency.
...
\midrule
POST /rag/query & User & Runs permission-safe retrieval and returns traceId, answer, citations, chunks, blocked document IDs, topK, status, and latency \\
...
The /app/rag page is designed as an operational workbench rather than a simple chatbot. It includes a question input area, topK selector, loading states, answer rendering, citation cards, document metadata badges, and permission-filter diagnostics. Each citation card displays document title, chunk snippet, similarity score, security level, and document link. When candidate documents are retrieved but blocked by permission rules, the page shows the blocked count without exposing blocked content. If the user needs a restricted document, the access-request action creates a RAG\_APPLY notification. After an administrator or department administrator approves it, RagPermissionService includes the approved document in the user's permission snapshot, so later retrieval can use that document as evidence. Copilot intent routing can redirect RAG-like questions to /app/rag?query=..., so a user can begin from the Copilot entry point and continue in the specialized RAG workbench with the question pre-filled.
