...
The existing sys\_document table remains the document master table, storing title, content, dept\_id, and security\_level. Five RAG metadata tables support the end-to-end pipeline. rag\_knowledge\_base stores knowledge-base metadata. rag\_source\_document records uploaded source files and parse/index status. rag\_document\_chunk stores chunk-level records, vector IDs, embedding profile metadata, content hashes, and index status. rag\_query\_log records user queries, retrieved and blocked documents, generated answers, latency metrics, and embedding profile information. rag\_index\_task records rebuild, indexing, and deletion task status.

\subhead{5.3.3 Milvus Vector Database Integration}

Milvus is used as the vector database. Docker Compose is extended with Milvus Standalone and its dependencies. On startup, rag-agent initializes a Milvus collection with dimension matching the active embedding profile. The default local BGE-M3 profile uses 1024-dimensional embeddings, COSINE similarity, and HNSW indexing. The Milvus collection schema is configured with fields: vector\_id (primary key), document\_id, chunk\_id, chunk\_index, dept\_id, security\_level, content\_hash, chunk\_text, and embedding (FloatVector).

\subhead{5.3.4 Document Chunking Strategy}

Documents are processed with a structure-aware chunking strategy. Documents are first split by Markdown headings and paragraph boundaries to preserve document hierarchy. When a segment exceeds the target chunk size, it is split into overlapping chunks. The default configuration uses 700 tokens with a 100-token overlap. Each chunk inherits the metadata of the source document: document ID, department ID, and security level.

\subhead{5.3.5 Embedding Generation}

Both document chunks and user questions need to be embedded into a shared vector space. The EmbeddingClient interface defines single-text and batch embedding methods, and its implementation supports a local HTTP embedding worker, external embedding APIs, or a mock profile for development checks. The active embedding profile controls the model name, vector dimension, Milvus collection, and readiness status through configuration.

\subhead{5.3.6 Permission Security Filtering}

Permission filtering ensures that the retrieved answers will never disclose the content of document beyond the user's authorized scope. The platform applies the existing access control model before answer generation. First, the system computes the set of document IDs the user is allowed to access based on department membership, security clearance, admin role, and approved RAG\_APPLY notifications. Second, vector retrieval obtains a broader candidate set from Milvus. Third, the candidate chunks are filtered against the allowed document list before they can enter the LLM prompt. Retrieved-but-blocked document IDs are recorded in the query log for auditability.

\subhead{5.3.7 Semantic Retrieval Flow}

The retrieval flow first encodes the user's question with the same embedding model as the document indexing. Milvus performs an ANN (Approximate Nearest Neighbor) search against the active embedding profile's collection and returns a candidate list larger than the requested top-K. The service then applies permission filtering and keeps the highest-scoring allowed chunks. The default profile uses COSINE similarity between the query vector and document vectors.

\subhead{5.3.8 LLM Answer Generation with Citations}

The LLM (configured through RAG\_LLM\_API\_KEY, RAG\_LLM\_BASE\_URL, RAG\_LLM\_MODEL) is invoked by rag-agent through an OpenAI-compatible chat-completions interface to generate responses. The topK retrieved chunks are assembled into a structured prompt including the question, permission-safe context chunks with source metadata, and instructions for generating a concise answer with citations. The response includes answer text and a list of referenced sources. The frontend renders citations with document and chunk identifiers, snippets, similarity scores, and related source metadata.

\subhead{5.3.9 RAG API Endpoints}

The RAG agent maps the RESTful API endpoints, and all endpoints are protected by Gateway-level JWT authentication. POST /rag/query is the core Q\&A endpoint, accepting \{question, topK\} and returning answer text, citations, retrieved chunks, retrieved document IDs, blocked document IDs, status, and latency. POST /rag/index/rebuild triggers a full re-indexing task. POST /rag/index/document/\{id\} indexes a single document with chunk-clean-rebuild logic. DELETE /rag/index/document/\{id\} deletes all chunks and vectors belonging to a document. GET /rag/health returns service, Milvus, embedding, and LLM readiness. Additional endpoints cover embedding profile management, accessible-document snapshots, knowledge-base management, document upload, and access requests.

\subhead{5.3.10 Task Service Integration}

The RAG capability has been integrated into the task-service through actual rag-agent calls. When a RAG-type task is submitted, the task-service calls the RAG agent's POST /rag/query endpoint and pushes execution progress through WebSocket, including retrieval, grounded answer generation, and completion stages. The RAG response is saved in task\_record.output, enabling the Task Center and Copilot to use the same RAG capability through the existing orchestration framework.
...
The indexing lifecycle follows a fixed sequence: source document registration, document parsing, structure-aware chunk generation, embedding creation under the active profile, chunk metadata persistence in MySQL, vector upsert into Milvus, and index-task status update. Query-time audit data is stored separately in rag\_query\_log so that administrators can inspect which documents were retrieved, which documents were blocked, which embedding profile was used, and how long the request took.

\subhead{5.3.13 Docker Deployment Configuration}

Docker Compose is extended with new services: rag-agent (Spring Boot on port 8085), Milvus Standalone, etcd (Milvus metadata coordination), MinIO (Milvus object storage), and rag-worker (local embedding model server). New environment variables include: \texttt{RAG\_SERVICE\_URI}, \texttt{MILVUS\_HOST}, \texttt{MILVUS\_PORT}, \texttt{RAG\_EMBEDDING\_PROVIDER}, \texttt{RAG\_EMBEDDING\_MODEL}, \texttt{RAG\_EMBEDDING\_DIM}, \texttt{RAG\_CHUNK\_SIZE\_TOKENS}, \texttt{RAG\_CHUNK\_OVERLAP\_TOKENS}, \texttt{RAG\_LLM\_API\_KEY}, \texttt{RAG\_LLM\_BASE\_URL}, \texttt{RAG\_LLM\_MODEL}, \texttt{RAG\_MILVUS\_COLLECTION}, \texttt{RAG\_VECTOR\_METRIC}, and \texttt{RAG\_VECTOR\_INDEX}. The services include dependency ordering so that Milvus, MinIO, and the embedding worker start before rag-agent. The goal is a single \texttt{docker compose up --build -d} command that brings the complete platform online.

\subhead{5.3.14 Technical Challenges and Testing Strategy}

(1) Permission security: RAG retrieval must not leak document content through answers. Defense in depth with pre-retrieval access-scope computation, broader candidate retrieval, and post-retrieval permission re-verification prevents unauthorized chunks from entering the prompt.
