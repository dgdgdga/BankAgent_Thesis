...
\subsubsection{Enterprise RAG as Evidence Construction}
...
Therefore, this section formulates the RAG Agent as a \textbf{three-stage evidence construction framework}. The first stage is provenance-preserving indexing, which converts source documents into traceable chunks and vectors before query time. The second stage is access-aware candidate retrieval, which uses the active embedding profile and user context to retrieve a ranked candidate set. The third stage is citation-grounded synthesis, which converts selected chunks into an answer, citations, and an audit record. The technical focus is an end-to-end enterprise implementation that makes these three stages explicit, observable, and reusable by other agents in the platform.

Formally, for a user $u$, question $q$, and active embedding profile $p$, the system first computes the accessible document set $A(u)$ from role, department, clearance level, and temporary approvals. Let $C_p$ denote the chunk set indexed under profile $p$, $\mathrm{doc}(c)$ denote the source document of chunk $c$, and $\mathrm{valid}(c,p)$ denote that the chunk belongs to the active profile and has current metadata. The final evidence set is selected as:

\[
E_k(q,u,p)=\mathrm{TopK}_{\mathrm{sim}(e_p(q),e_p(c))}
\{c \in C_p \mid \mathrm{doc}(c)\in A(u) \land \mathrm{valid}(c,p)\}
\]
...
Table \ref{tab:rag-evidence-framework} summarizes the three-stage framework used by the implementation.
...
\small\singlespacing
\centering
\caption{Three-Stage Evidence Construction Framework in the RAG Agent}
\label{tab:rag-evidence-framework}
\vspace{2mm}

\begin{tabularx}{\textwidth}{>{\RaggedRight\arraybackslash}p{0.20\textwidth}>{\RaggedRight\arraybackslash}p{0.26\textwidth}>{\RaggedRight\arraybackslash}p{0.31\textwidth}>{\RaggedRight\arraybackslash}X}
\toprule
\textbf{Stage} & \textbf{Problem Addressed} & \textbf{System Mechanism} & \textbf{Output} \\
\midrule
...
\textbf{Lane} & \textbf{Processing Path} & \textbf{Platform Output} \\
\midrule
Knowledge preparation &
Document upload $\rightarrow$ Tika parser $\rightarrow$ block-aware chunker $\rightarrow$ EmbeddingClient $\rightarrow$ VectorStoreService &
chunk metadata + Milvus vectors \\
\addlinespace[1mm]
Evidence retrieval &
JWT headers $\rightarrow$ RagPermissionService $\rightarrow$ query embedding $\rightarrow$ scoped Milvus over-fetch $\rightarrow$ evidence selection &
selected chunks + similarity scores \\
\addlinespace[1mm]
Grounded synthesis &
selected evidence $\rightarrow$ prompt builder $\rightarrow$ LLM $\rightarrow$ citation builder $\rightarrow$ response mapper &
answer + citations + traceId \\
\addlinespace[1mm]
Shared controls &
embedding profile, vector collection, document lifecycle state, department scope, approved RAG\_APPLY records &
query log + profile/access trace \\
\end{tabularx}
\end{minipage}
}
\caption{RAG Agent Architecture --- Knowledge Preparation, Evidence Retrieval, and Grounded Synthesis}
...
rag\_source\_document & source location, parse state, index state, error state & Tracks uploaded files and parsing/index lifecycle \\
rag\_document\_chunk & chunk text, content hash, vector linkage, embedding profile, index state & Connects MySQL metadata to Milvus vector records \\
rag\_query\_log & user, question, answer, evidence summary, latency, embedding profile & Audit trail for answer diagnosis, citation review, quality monitoring, and permission review \\
rag\_index\_task & task category, target document, execution state, progress, error state & Operational tracking for rebuild, single-document index, and deletion jobs \\
...
FUNCTION indexDocument(documentId: Long): IndexResult
    ...
    TRY
        ...
        profile = embeddingConfigService.getCurrentConfig()
        doc = ensureDocumentTextReady(doc, forceParse = false)

        // Clean rebuild for the active profile
        vectorStoreService.deleteByDocumentId(documentId)
        chunkRepository.hardDeleteByDocumentIdAndProfile(
            documentId, profile.name)

        chunks = MarkdownDocumentChunker.split(doc.content)
        vectorRecords = []
        insertedChunks = []
        ...
        FOR EACH chunk IN chunks:
            ...
            chunkRow = chunkRepository.insert(
                ...
                vectorCollection = profile.collection,
                indexStatus = "RUNNING",
                deptId = doc.deptId,
                securityLevel = doc.securityLevel)
            insertedChunks.add(chunkRow)
        ...
        chunkRepository.markAllIndexed(insertedChunks, "SUCCESS")
...
\textbf{Candidate discovery} & \textbf{Evidence selection} & \textbf{Grounded response} \\
\midrule
Milvus receives the query vector and access context, then returns over-fetched chunks with document IDs, chunk IDs, similarity scores, and metadata. &
RagQueryService keeps chunks that have sufficient semantic relevance, valid provenance, and current access eligibility. &
Selected chunks become numbered context blocks for the LLM, citation cards for the frontend, and retrieved-document records in the audit log. \\
\addlinespace[2mm]
Candidate discovery favors recall by requesting more chunks than the final top-K. &
Evidence selection converts raw candidates into a compact and inspectable evidence set. &
The response carries answer text, citations, traceId, profile metadata, latency, and excluded-document diagnostics. \\
\end{tabularx}
\end{minipage}
}
\caption{RAG Evidence Gate --- Candidate Chunks Become Citable Answer Evidence}
...
FUNCTION retrieveAllowedChunks(request, user): RetrievalResult
    ...
    topK = normalizeTopK(request.topK, default = 5, max = 20)
    profile = embeddingConfigService.getCurrentConfig()
    ...
    IF allowedDocIds IS EMPTY THEN
        RETURN RetrievalResult.empty("NO_CONTEXT")
    END IF

    IF embeddingConfigService.activeIndexStatus != "READY" THEN
        RETURN RetrievalResult.empty("NO_CONTEXT")
    END IF

    queryVector = embeddingClient.embed(request.question)
    candidateLimit = max(topK * 4, topK)
    candidates = vectorStoreService.search(
        profile.collection, queryVector, "COSINE",
        limit = candidateLimit,
        allowedDocumentIds = allowedDocIds)

    // Re-check document IDs after Milvus retrieval
    allowedChunks = []
    ...
    FOR EACH candidate IN candidates:
        ...
        END IF
        IF allowedChunks.size == topK THEN
            BREAK
        END IF
    END FOR
    ...
    RETURN RetrievalResult(traceId, allowedChunks, blockedDocIds, profile, topK)
END FUNCTION
\end{lstlisting}
\end{table}
...
FUNCTION generateGroundedAnswer(request, user): RagAnswer
    ...
    IF retrieval IS EMPTY THEN
        RETURN noContextAnswer(retrieval.status)
    END IF

    TRY
        prompt = buildSafePrompt(request.question, retrieval.allowedChunks)
        answer = llmClient.generate(prompt)
        status = "SUCCESS"
    CATCH llmError
        answer = buildFallbackAnswer(retrieval.allowedChunks, llmError)
        status = "LLM_FALLBACK"
    END TRY

    citations = citationBuilder.fromChunks(
        retrieval.allowedChunks)
    ...
    RETURN RagAnswer(answer, citations, retrieval.allowedChunks, status)
END FUNCTION
\end{lstlisting}
\end{table}
...
Failure handling is part of the query contract rather than an unhandled exception path. If the user has no accessible documents or the active index is not ready, the service returns a controlled no-context response. If LLM generation fails after evidence has already been retrieved, the service returns the selected snippets as a fallback answer, so the user still receives grounded retrieval evidence. If Milvus, the embedding endpoint, or another unexpected step fails, the query service preserves the trace identifier, writes the status and message into the query log, and returns a frontend-renderable failure response. This keeps the RAG workbench debuggable without exposing inaccessible document content.
...
\small\singlespacing
\centering
\caption{RAG Agent API Capability Groups}
...
\toprule
\textbf{Capability Group} & \textbf{Primary Role} & \textbf{Behavior} \\
\midrule
