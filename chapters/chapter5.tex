...
The RAG Agent addresses a different problem from a conventional document chatbot. In a normal RAG demonstration, the main objective is to retrieve semantically similar passages and use them as context for generation. In an enterprise banking setting, semantic relevance is necessary but not sufficient. A retrieved passage is useful only if it is also authorized for the current user, traceable to a source document, reproducible under a specific embedding profile, and visible to administrators for later audit. Therefore, the design treats RAG as a \textbf{constrained evidence selection} problem rather than a simple vector-search problem.
...
This formulation drives the rest of the architecture: permission evaluation is performed before and after vector retrieval; citations are built from the final evidence set rather than from model text; and every query stores enough metadata to reconstruct which documents were retrieved, blocked, and used.
...
\midrule
Retrieval objective & Return the most semantically similar chunks & Return the most relevant chunks subject to department, clearance, and approval constraints \\
Security boundary & Often handled outside the retrieval loop & Built into permission snapshot, scoped vector search, and post-retrieval verification \\
Grounding evidence & Context is mainly a prompt construction detail & Evidence is a first-class object with citations, chunk IDs, document IDs, scores, and audit logs \\
...
Operational readiness & Indexing is often a manual preprocessing step & Index tasks, startup readiness checks, and asynchronous rebuild status make indexing observable \\
\bottomrule
\end{tabularx}
\end{table}
...
The RAG Agent is implemented as an independent Spring Boot microservice on port 8085. Its role is not merely to wrap Milvus or expose a document-search endpoint. Instead, it coordinates the complete evidence lifecycle: document ingestion, structure-aware chunking, embedding, vector indexing, permission-constrained retrieval, grounded answer generation, citation construction, and query audit logging. The service is exposed through Spring Cloud Gateway under /api/rag/**. Gateway authentication remains the first security boundary: JwtAuthGlobalFilter validates the JWT token and forwards X-User-Id, X-User-Name, X-User-Dept-Id, X-User-Roles, and X-User-Security-Level headers to rag-agent.

Table \ref{tab:rag-agent-pipeline} summarizes the internal pipeline. Compared with the Code Agent, whose main risk is unsafe SQL execution, the RAG Agent's main risk is unauthorized knowledge leakage through retrieved context. Therefore, permission calculation, evidence selection, and audit logging are designed as core stages of the pipeline rather than auxiliary checks.
...
The retrieval step can therefore be understood as a three-stage decision process. First, the system computes a permission snapshot that defines what the user may know. Second, vector search ranks semantically relevant chunks within that accessible universe. Third, the service builds the final evidence set only from chunks that survive post-retrieval verification. This design makes access control part of the evidence-selection algorithm rather than a user-interface filter applied after the model has already seen the context.
...
The answer generation step uses an OpenAI-compatible chat-completions endpoint configured by the active AI provider. The prompt has three parts: the user question, a numbered context block containing permission-safe chunks, and answer rules. The answer rules require the model to answer only from the provided context, cite chunk identifiers, state that no evidence was found when the context is insufficient, and never infer content from blocked documents. This turns the LLM into a controlled synthesis component: it can phrase and summarize evidence, but it does not decide which documents it is allowed to read. The response returned to the frontend contains traceId, answer text, citations, retrieved chunks, retrieved document IDs, blocked document IDs, topK, status, and latency.
...
Evaluation focuses on both answer behavior and leakage prevention: admin users can retrieve high-clearance documents; standard users are restricted to same-department and clearance-allowed documents; low-clearance users cannot see confidential chunks; approved RAG\_APPLY requests expand access only within the configured TTL; document updates refresh the index; document deletion removes vectors; Milvus or LLM failures return controlled error states; and the frontend citation panel renders evidence without exposing blocked document content. In addition, embedding-profile tests verify that BGE-M3 and Qwen vectors are written to separate collections, that dimension mismatches are caught by health checks, and that index rebuild status is visible before users rely on a newly activated profile.
