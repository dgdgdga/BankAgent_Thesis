...
I built the platform's early-stage authentication and permission system with Spring Security, using stateless JWT tokens (HMAC-SHA256) and BCrypt password encoding to separate admin and regular user access. I configured the security filter chain to open only the login and registration endpoints while requiring authentication for all other routes, and defined standardized Result\textless T\textgreater response wrappers and DTOs.

For internationalization, I adapted all business and UI components to support three languages (Simplified Chinese, Traditional Chinese, and English) with dynamic switching through Vue3 and Element Plus i18n. This required reorganizing component-level text, validation messages, and document metadata into locale files.

Building on the existing team codebase, I integrated QQ Mail's SMTP service to deliver verification codes, storing them in Redis with a 5-minute TTL and a 60-second send rate limit, and wired the code verification into the registration flow.

I was responsible for the final report. I redrew all twelve system diagrams, standardized and expanded the technical tables, and synchronized the report with the latest codebase.

Figure redesign. I designed the first version of all figures, and after team discussion, I redrew the original dense, small-font figures into clean, wide-format vector graphics with a unified visual style. Every diagram maintained readability and visual hierarchy, ensuring clear communication of complex architectural concepts. A total of 12 figures were created and refined.

Table standardization. I converted all seven data tables into the three-line table format (1.5 pt top and bottom rules, a 0.75 pt rule below the header, no vertical or interior horizontal lines). I also suggested moving long pseudocode blocks to standalone tables---both to maintain formatting consistency and to communicate algorithms more clearly to readers.
