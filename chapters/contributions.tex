...
\textbf{(1) RAG microservice and API design}

I independently built the \texttt{rag-agent} microservice as a standalone Spring Boot service running on port 8085 and integrated it into the platform through the Gateway route \texttt{/api/rag/**}. I implemented the main controller, service, DTO, entity, mapper, and configuration layers, and exposed the core RAG APIs including \texttt{/rag/query}, \texttt{/rag/health}, \texttt{/rag/health/embedding}, \texttt{/rag/index/rebuild}, \texttt{/rag/index/documents/status}, \texttt{/rag/embedding/profiles}, and \texttt{/rag/embedding/active}. This made the RAG Agent a dedicated intelligent agent module in the platform, rather than a separate experimental component.

\textbf{(2) RAG retrieval pipeline}
...
\textbf{(3) RAG database and metadata model}

I designed and implemented the RAG metadata model around \texttt{rag\_document\_chunk}, \texttt{rag\_query\_log}, \texttt{sys\_document}, and \texttt{sys\_config}. I added and maintained fields such as \texttt{vector\_id}, \texttt{embedding\_profile}, \texttt{embedding\_model}, \texttt{vector\_collection}, \texttt{content\_hash}, \texttt{chunk\_index}, \texttt{index\_status}, and \texttt{last\_indexed\_at}. These fields allow each retrieved chunk to be traced back to its source document, embedding model, active profile, and Milvus collection, and they also support index rebuilds, profile switching, and document index status inspection.

\textbf{(4) Permission-aware retrieval}

I aligned the RAG Agent with the platform’s RBAC, department, and clearance-level security model. The service uses identity headers such as \texttt{X-User-Name} and \texttt{X-User-Roles} propagated by the Gateway, then checks the user’s department and clearance level before allowing document content into the retrieval context. I also implemented blocked-document tracking and controlled \texttt{NO\_CONTEXT} responses, preventing unauthorized or cross-department documents from appearing in generated answers and citations.

\textbf{2. Vector Store and Embedding}

\textbf{(1) Milvus integration}
...
\textbf{(2) Local embedding worker}

I built and tested a local Python \texttt{rag-worker} using the BGE-M3 embedding model. The worker exposes an HTTP \texttt{/embed} endpoint and returns 1024-dimensional embeddings. The Java RAG Agent calls this worker through HTTP, keeping the Spring Boot service independent from Python model runtime details. I also handled local worker setup, including virtual environment creation, dependency installation, model download, Hugging Face cache behavior, startup delay, health checks, and embedding dimension verification.

\textbf{(3) Qwen cloud embedding API support}

I extended the embedding layer to support Qwen \texttt{text-embedding-v4} through a cloud API. I added configuration for provider type, endpoint, API key, model name, vector dimension, timeout, and collection name. This allows the RAG Agent to support both local BGE-M3 embeddings and cloud-based Qwen embeddings.

\textbf{(4) Embedding profile isolation}

I designed the embedding profile mechanism to keep different embedding models isolated. Each profile has its own provider, model, dimension, endpoint, active status, index status, and Milvus collection. For example, \texttt{local-bge-m3} stores vectors in \texttt{rag\_document\_chunks\_bge\_m3}, while \texttt{qwen-v4} stores vectors in \texttt{rag\_document\_chunks\_qwen\_v4\_1024}. I implemented APIs for listing profiles, checking the active profile, testing providers, activating profiles, and rebuilding the index after switching profiles. This avoids mixing incompatible vector spaces and keeps retrieval results technically valid.

\textbf{3. Knowledge Base and Document Management}

\textbf{(1) Document storage collaboration}
...
\textbf{(2) Document chunking and indexing}
...
\textbf{(3) Index rebuild workflow}

I implemented the active-profile index rebuild workflow. When documents are added or changed, or when the administrator switches embedding providers, the RAG Agent can rebuild the index so that MySQL chunk metadata and Milvus vectors match the current embedding profile. I verified the rebuild process through \texttt{/rag/health}, \texttt{/rag/index/documents/status}, direct MySQL checks, and actual RAG queries.

\textbf{4. Frontend and Integration}

\textbf{(1) RAG frontend interface alignment}
...
\textbf{(2) Administrator embedding provider switch}

I proposed and helped design the administrator-side embedding provider switch. The frontend can expose local BGE-M3 and Qwen \texttt{text-embedding-v4} as selectable options, while sensitive information such as API keys remains on the backend side. I also clarified the rule that switching providers requires rebuilding the index because each embedding profile uses a separate vector collection.

\textbf{(3) Cross-service integration}
...
\textbf{(1) Local environment and service verification}
...
\textbf{(3) End-to-end RAG testing}

I tested the complete RAG workflow through \texttt{/rag/health}, \texttt{/rag/health/embedding}, \texttt{/rag/index/rebuild}, \texttt{/rag/index/documents/status}, \texttt{/rag/embedding/active}, and \texttt{/rag/query}. I also checked MySQL directly to confirm that chunk metadata, vector IDs, embedding profiles, embedding models, and collection names were stored correctly. After rebuilding the active profile, I confirmed that RAG queries returned successful results with citations and permission-safe retrieved chunks.

\textbf{(4) Documentation and team communication}
